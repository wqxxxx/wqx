	...
	<view class="contact page-pagination" :class="size == 'large' ? 'size-large' : size == 'small' ? 'size-small' : ''">
			...
			<view class="page-scroll">
				<view class="page-scroll-child">
					...
					<view
						class="pag-btn start"
						:style="{ color }"
						:class="[nowPage == 1 ? 'btn-ban' : '', btnText ? 'btn-text' : '']"
						v-if="showAround && !numAround"
						@click="clickStart"
					>
					...
					<view v-if="mode == 'simple'" class="page-num simple">
						<text :style="{ color }" style="margin-right: 10rpx;">{{ nowPage }}</text>
						/ {{ pageNum }}
					</view>
					<!-- 复杂模式 -->
					<view class="page-num" v-else>
						...
						<template v-else>
							...
							<view
								class="pag-btn"
								:style="{ color, backgroundColor: nowPage == 1 ? color : '' }"
								v-if="numAround"
							...
							<!-- 左侧省略号 -->
							<view class="pag-btn ellipsis-btn" :style="{ color }" v-show="(forceEllipses && getFirstPage != 1) || (numAround && getFirstPage != 2)">
							...
							<template v-for="(p, i) in showPageSize">
								...
								<template v-else>
									...
									<view
										:key="i"
										v-show="!forceEllipses && !numAround"
										class="pag-btn"
										:style="{ color, backgroundColor: getFirstPage + i == nowPage ? color : '' }"
										:class="getFirstPage + i == nowPage ? 'active' : ''"
										@click="clickPage(getFirstPage + i)"
							...
							<view
								class="pag-btn"
								:style="{ color, backgroundColor: nowPage == pageNum ? color : '' }"
								v-if="numAround"
								v-show="getFirstPage < pageNum - (showPageSize - 3)"
								:class="nowPage == pageNum ? 'active' : ''"
					...
					<view
						class="pag-btn end"
						:style="{ color }"
						:class="[pageNum <= nowPage ? 'btn-ban' : '', btnText ? 'btn-text' : '']"
						v-if="showAround && !numAround"
						@click="clickEnd"
					>
		...
		<view class="page-info">
			...
			<view class="page-info-con" v-if="showPageInfo">
				...
				<view>
					当前页 {{ nowPage }}/{{ pageNum }}
					<text v-if="showGoPage">，</text>
				</view>
			</view>
			<view class="page-go" v-if="showGoPage">
				前往第
				<template v-if="trigger == 'blur'">
					<input class="page-input" type="text" v-model="inputPage" @blur="goPage" @input="onInput" />
					页
				</template>
				<template v-else>
					<input class="page-input" type="text" v-model="inputPage" @input="onInput" />
					页
...
export default {
	...
	computed: {
		...
		pageNumArr() {
			//解决uni-app某些版本中，存在v-for中循环数字时从0开始
			var pageNumArr = [];
